  ...
  try {
    ...
    if (userError) {
      ...
      return NextResponse.json(
        { error: "Failed to fetch user credits. Please try again." },
        { status: 500 },
      );
    }
      ...
      .map((file, index) => {
        return (
          ...
          (index < audioFiles.length - 1
            ? `file '${path.resolve(silentAudioPath)}'\n`
            : "")
        );
      })
      .join("");
    ...
    return new Promise((resolve, reject) => {
      ffmpeg()
        ...
        .outputOptions([
          ...
          "-b:a",
          "192k",
        ]) // Re-encode to ensure all files are uniform
        ...
        .on("end", async () => {
          ...
          if (!error) {
            ...
            const { data: meditationData, error: meditationError } =
              await supabase
                .from("meditations")
                .insert({
                  user_id: userId,
            ...
            if (meditationError) {
              console.error(
                "Error inserting meditation into table:",
                meditationError,
              );
              reject(
                NextResponse.json(
                  { error: "Failed to save meditation. Please try again." },
                  { status: 500 },
                ),
              );
            } else {
              ...
              resolve(
                NextResponse.json({
                  message: "Meditation generated and stored successfully.",
                  meditationId,
                }),
              );
            }
            ...
            console.error(
              "Error uploading file to Supabase private bucket:",
              error,
            );
            reject(
              NextResponse.json(
                {
                  error:
                    "Failed to upload meditation to Supabase. Please try again.",
                },
                { status: 500 },
              ),
            );

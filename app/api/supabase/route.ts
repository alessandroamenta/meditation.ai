...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (meditationError) {
      ...
      return NextResponse.json(
        { error: "Failed to retrieve meditation data." },
        { status: 500 },
      );
    }
    ...
    const { data: signedUrlData, error: signedUrlError } =
      await supabase.storage
        ...
        .createSignedUrl(
          `user_${userId}/${meditationData.audio_path}`,
          60 * 60,
        );
    ...
    return NextResponse.json(
      { error: "Failed to retrieve meditation." },
      { status: 500 },
    );
  }
}
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    if (deleteError) {
      ...
      return NextResponse.json(
        { error: "Failed to delete meditation audio." },
        { status: 500 },
      );
    }
    ...
    if (deleteMeditationError) {
      ...
      return NextResponse.json(
        { error: "Failed to delete meditation record." },
        { status: 500 },
      );
    }
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: "Failed to update meditation name." },
        { status: 500 },
      );
    }
    ...
    return NextResponse.json(
      { error: "Failed to rename meditation." },
      { status: 500 },
    );
  }
}

...
export async function GET() {
  try {
    const { data: openaiVoiceSamples, error: openaiError } = await supabase.storage
      .from('voice-samples')
      .list('openai', {
        limit: 100,
        offset: 0,
    ...
    const { data: elevenlabsVoiceSamples, error: elevenlabsError } = await supabase.storage
      .from('voice-samples')
      .list('elevenlabs', {
        limit: 100,
        offset: 0,
    ...
    if (openaiError || elevenlabsError) {
      console.error('Error retrieving voice sample URLs:', openaiError || elevenlabsError);
      return NextResponse.json({ error: 'Failed to retrieve voice sample URLs' }, { status: 500 });
    }
